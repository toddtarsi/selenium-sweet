...
const makeDoSubs = function(script, step, name, userParams, used_vars, lang_info) {
...
const canExport = function(lang_info, stepType) {
...
const createLangFormatter = function(lang_info) {
  return {
    name: lang_info.name,
    ...
    canExport: function(stepType) {
      return canExport(lang_info, stepType);
    },
    nonExportables: function(script) {
      var nes = [];
      for (var i = 0; i < script.steps.length; i++) {
        var step = script.steps[i];
        if (nes.indexOf(step.type.name) == -1 && !canExport(lang_info, step.type)) {
...
const createDerivedLangFormatter = function(original, lang_info_diff) {
  var new_info = {};
